...
func (r *AWSManagedMachinePoolReconciler) reconcileNormal(
	...
	if machinePoolScope.IsBYOLaunchTemplate() {
		...
		if lt.VersionNumber != nil {
			...
			// Use AWS symbolic version alias to avoid requiring EC2 permissions for "latest version" discovery.
			machinePoolScope.SetLaunchTemplateLatestVersionStatus("$Latest")
		}
